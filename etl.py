...
#df_spark = spark.read.csv('/home/hadoop/immigration_data_sample.csv', header=True, inferSchema=True)
#df_spark = spark.read.csv('s3a://ma2516-immigration-data/immigration_data_sample.csv', header=True, inferSchema=True)
df_spark = spark.read.format('com.github.saurfang.sas.spark').option("inferSchema", "true").load('s3a://ma2516-immigration-data/i94_feb16_sub.sas7bdat')
